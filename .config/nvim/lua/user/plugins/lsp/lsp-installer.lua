...
function M.init(use, plugin_fn)
  jdtls = jdtls or require('user.plugins.lsp.jdtls')
  lsp_config = lsp_config or require('user.plugins.lsp.lsp-config')
...
function M.setup()
  ...
  lsp_installer.on_server_ready(function(server)
    local opts = {
      on_attach = require('user.plugins.lsp.lsp-config').get_on_attach(),
      capabilities = require('user.plugins.lsp.lsp-config').get_capabilities(),
    }
    ...
    if M.server_settings[server.name] then
      local sv_opts = require('user.plugins.lsp.settings.' .. server.name)
